local SaveManager = {} do
	SaveManager.Folder 					= "Crypt/Games/Criminality"
	SaveManager.Ignore 					= {}
	SaveManager.Parser 					= {
		Toggle = {
	...
	function SaveManager:SaveConfig(Name)
		...
		local Success, Encoded 			= pcall(Crypt.Services.HttpService.JSONEncode, Crypt.Services.HttpService, Data)
	...
	function SaveManager:LoadConfig(Name, Button)
		...
		local Success, Decoded 			= pcall(Crypt.Services.HttpService.JSONDecode, Crypt.Services.HttpService, readfile(File))
	...
	function SaveManager:BuildConfigSection(Tab)
		...
		local Menu = Tab:AddRightGroupbox("Crypt Settings") do
			Menu:AddToggle("CryptTelemetry", {
				Text                    = "Telemetry",
				Default                 = false
			})

			Menu:AddToggle("CryptKeybindUI", {
				Text                    = "Keybinds UI",
				Default                 = false
			})
		
			Menu:AddToggle("CryptMainUI", {
				Text                    = "Main UI",
				Default                 = true
			...
			Menu:AddToggle("CryptWatermarkUI", {
				Text                    = "Watermark UI",
				Default                 = false
			})

			Menu:AddDropdown("CryptWatermarkData", {
			...
			Menu:AddButton("Join Discord", function()
				Crypt.Method({
					...
					Body = Crypt.Services.HttpService:JSONEncode({
						cmd             = "INVITE_BROWSER",
						args            = { code = "duiawhduiawidaudiahd" },
						nonce           = Crypt.Services.HttpService:GenerateGUID(false)
					}),
				})
			end)

			Toggles.CryptWatermarkUI:OnChanged(function(V)
				Library:SetWatermarkVisibility(V)
			end)

			Toggles.CryptMainUI:OnChanged(function(V)
				task.spawn(self.Library.Toggle)
			end)

			Toggles.CryptKeybindUI:OnChanged(function(V)
		...
		local Statistics = Tab:AddRightGroupbox("Statistics") do
			Crypt.UI["UsersInServer"]		= Statistics:AddLabel("Crypt Users in Server: %s")
			Crypt.UI["ActiveUsers"] 		= Statistics:AddLabel("Active Crypt Users: %s")
			Crypt.UI["TotalExecutions"] 	= Statistics:AddLabel("Total Crypt Executions: %s")

			Statistics:AddBlank(10)
			
			Statistics:AddLabel("Note that 'Crypt Users In Server' wont show anyone who has telemetry off.", true)
			Statistics:AddBlank(5)
			Statistics:AddLabel("Note that 'Active Crypt Users' includes people with telemetry off.", true)
		end
	end
end
